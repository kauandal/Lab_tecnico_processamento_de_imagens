...
def test_lab_json_valido():
    ...
    assert data["lab"] == "m1.3"
    assert data["language"] == "python"
    assert data["student"]["name"]


def test_kernels_do_projeto_presentes():
    """O M1.3 usa kernels lidos de arquivo, entao o diretorio nao pode estar vazio."""
    arquivos = list((PROJECT_ROOT / "kernels").glob("*.txt"))

    assert arquivos, "nenhum kernel em kernels/"
    assert (PROJECT_ROOT / "kernels" / "identity_3x3.txt").exists()
